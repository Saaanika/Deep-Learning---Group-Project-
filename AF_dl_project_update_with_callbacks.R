...
ff_model1history <- ff_model1 %>% fit(
  ...
  callbacks = list(make_early_stop())    #early stopping restores best epoch weights automatically
)
...
ff_model12history <- ff_model12 %>% fit(
  ...
  batch_size = 128,        
  validation_data = list(x_val, y_val),
  callbacks = list(make_early_stop())
)
...
# Getting the best val accuracy across all epochs for each model
# with early stopping + restore_best_weights, the model in memory already has the best weights
...
# How many epochs each model actually trained for (early stopping may have cut it short)
epochs_trained <- sapply(all_histories, function(h) length(h$metrics$val_accuracy))
names(epochs_trained) <- model_names

# Also getting the final epoch val accuracy
final_val_acc <- sapply(all_histories, function(h) tail(h$metrics$val_accuracy, 1))
names(final_val_acc) <- model_names

# Printing val accuracy for each model
cat("\nValidation Accuracy Summary:\n")
for (i in seq_along(model_names)) {
  cat(model_names[i], "- Best:", round(best_val_acc[i], 3),
      " at Epoch:", best_epoch[i],
      " (trained", epochs_trained[i], "epochs)\n")
}
...
# Note: some models may have fewer than 20 epochs due to early stopping
...
# ------------------------------------------------------------------------------
# Part Eleven: Accuracy and Ranked Probability Score (RPS) Evaluation on Test Data
# ------------------------------------------------------------------------------
...
# Metrics will be calculated on the completely unseen test dataset
# Because early stopping with restore_best_weights = TRUE was used, each model already has its best epoch weights
...
# Evaluating the original models directly (early stopping already restored best weights)
all_models <- list(
...
acc_test <- numeric(length(all_models))
rps_test <- numeric(length(all_models))


# Computing RPS and accuracy for each model on the test set 
for (i in seq_along(all_models)) {
  
  # Predicted probabilities from softmax output
  pred_probs <- predict(all_models[[i]], x_test) 
  ...
  rps_test[i] <- rps_score(y_test, pred_probs)
}
...
# ------------------------------------------------------------------------------
# Part Twelve: Saving Results
# ------------------------------------------------------------------------------

# Saving everything so we don't have to retrain
save(
  all_histories, all_models, model_names,
  best_val_acc, best_epoch, epochs_trained, final_val_acc,
  results_test, acc_test, rps_test,
  file = "project_results.RData"
)
cat("\nAll results saved to project_results.RData\n")

# to reload later: load("project_results.RData")